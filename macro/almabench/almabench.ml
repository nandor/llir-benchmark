...
let test_loops  = if Array.length Sys.argv >= 2 then int_of_string Sys.argv.(1) else 5
let test_length = 36525
...
(*    Printf.printf "%d %.2f %.2f\n%!" p position.(0) position.(1)*)
  ...
  (* Benchmark *)
  for _i = 0 to test_loops - 1 do
    jd.(0) <- j2000;
    jd.(1) <- 0.0;
    for _n = 0 to test_length - 1 do
      jd.(0) <- jd.(0) +. 1.0;
      for p = 0 to 7 do
  planetpv jd p pv;
  radecdist pv position;
