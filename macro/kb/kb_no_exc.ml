  ...
  match (t1, t2) with
    Var v, _ ->
      if List.mem_assoc v subst then
        if t2 = List.assoc v subst then Some subst else None
      else
        Some ((v, t2) :: subst)
  | Term(op1,sons1), Term(op2,sons2) ->
      if op1 = op2
      then fold_left2_opt match_rec subst sons1 sons2
      else None
  | _ -> None
...
let can_match l m =
  match matching l m with Some _ -> true | None -> false
...
let _ =
  for i = 1 to (try int_of_string(Array.get Sys.argv 1) with _ -> 20) do
