...
let neuterize : type a. a key -> public key = function
  ...
  | Sk sk ->
    ...
    assert (pk_of_sk (Bigstring.of_bytes sk) pk);
...
let sign (Sk sk) msg =
  let signature = Bigstring.create pk_size in
  match sign (Bigstring.of_bytes sk) (Bigstring.of_bytes msg) signature (to_curve secp256r1) with
  | true -> Some (Bigstring.to_bytes signature)
  | false -> None
